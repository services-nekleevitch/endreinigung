// Zürich Endreinigung — multi-step calculator (vanilla JS state machine)
// Keep pricing in sync with EndReinigung/Services/PriceCalculator.cs
...
(function () {
    ...
    // ===== Price data (mirror of PriceCalculator.cs) =====
    const APARTMENT_PRICES = {
        '1': 500, '1.5': 550, '2': 600, '2.5': 650, '3': 800, '3.5': 850,
        '4': 920, '4.5': 970, '5': 1100, '5.5': 1150, '6': 1300, '6.5': 1350
    };
    const HOUSE_PRICES = {
        '3': 1299, '3.5': 1799, '4': 1599, '4.5': 2099, '5': 1899, '5.5': 2399,
        '6': 2199, '6.5': 2699, '7': 2399, '7.5': 2899, '8': 2599, '8.5': 3099
    };
    const TOGGLE_PRICES = { basement: 50 };
    const COUNTABLE_PRICES = {
        balcony: 45, utility_balcony: 35, bath: 65, wc: 40,
        carpet: 120, balcony_pressure: 50, garage_pressure: 65
    };
    const EXTRA_LABELS = {
        basement: 'Estrich',
        balcony: 'Extra Balkon / Terrasse',
        utility_balcony: 'Nebenbalkon',
        bath: 'Anzahl Badezimmer',
        wc: 'Separates WC',
        carpet: 'Teppich Shampoo-Reinigung',
        balcony_pressure: 'Balkon / Terrasse Hochdruckreinigung',
        garage_pressure: 'Garage / Carport Hochdruckreinigung'
    };
    // C# property names for hidden inputs
    const COUNTABLE_FIELD = {
        balcony: 'Balcony', utility_balcony: 'UtilityBalcony', bath: 'Bath',
        wc: 'Wc', carpet: 'Carpet', balcony_pressure: 'BalconyPressure',
        garage_pressure: 'GaragePressure'
    };
    ...
    const state = {
        ...
        selectedRoom: '',
        zipCode: '',
        selectedAddOns: new Set(),   // currently: just 'basement'
        extraCounts: {
            balcony: 0, utility_balcony: 0, bath: 0, wc: 0,
            carpet: 0, balcony_pressure: 0, garage_pressure: 0
        },
        cleaningDate: '',
        handoverDate: '',
        handoverTime: '',
        handoverNotFixed: false,
        paymentMethod: '',
    ...
    // Mirror of PriceCalculator.getDefaultBathroomMapping
    ...
    function renderProgress() {
        const shown = Math.min(state.step, 4);
        for (let i = 1; i <= 4; i++) {
    ...
    function renderRoomGrid() {
        ...
        // Property icon in step 1 header
        const apartmentIcon = $('[data-property-icon="apartment"]');
        const houseIcon = $('[data-property-icon="house"]');
        if (apartmentIcon) apartmentIcon.classList.toggle('hidden', state.propertyType !== 'apartment');
        if (houseIcon) houseIcon.classList.toggle('hidden', state.propertyType !== 'house');
    ...
    function renderStep3() {
        ...
        const typeLabel = state.propertyType === 'house' ? 'Haus' : 'Wohnung';
        let html = '';
        html += `<div class="flex justify-between"><span class="text-muted-foreground">${typeLabel}</span><span class="font-medium">${getRoomLabel(state.selectedRoom)}</span></div>`;
        ...
        if (hasExtras) {
            ...
            state.selectedAddOns.forEach((id) => {
                html += `<li class="flex justify-between"><span class="text-muted-foreground">${EXTRA_LABELS[id]}</span><span class="font-medium">+CHF ${TOGGLE_PRICES[id]}</span></li>`;
            });
            Object.keys(state.extraCounts).forEach((key) => {
                const count = state.extraCounts[key];
                if (!count) return;
                const p = COUNTABLE_PRICES[key];
                const unit = key === 'carpet' ? ` (${count} Zimmer)` : ` (${count}×)`;
                html += `<li class="flex justify-between"><span class="text-muted-foreground">${EXTRA_LABELS[key]}${unit}</span><span class="font-medium">+CHF ${count * p}</span></li>`;
            });
            html += `</ul></div>`;
        }
    ...
    function renderStep4Summary() {
        ...
        if (room) room.textContent = `${typeLabel} • ${getRoomLabel(state.selectedRoom)}`;
    ...
    function validateStep3() {
        ...
        const handoverErr = $('[data-handover-date-error]');
        let ok = true;
        ...
        if (!state.cleaningDate) {
            ...
            cleaningErr.classList.add('hidden');
        }

        if (state.handoverDate && state.cleaningDate && state.handoverDate < state.cleaningDate) {
            if (handoverErr) { handoverErr.textContent = 'Übergabe muss nach dem Reinigungstermin liegen.'; handoverErr.classList.remove('hidden'); }
            ok = false;
        } else if (handoverErr) {
            handoverErr.classList.add('hidden');
        }
        return ok;
    }
    ...
    function wireStep3() {
        const cleaning = $('#calc-cleaning-date');
        const today = new Date().toISOString().split('T')[0];
        if (cleaning) {
            cleaning.min = today;
            cleaning.addEventListener('change', () => {
                state.cleaningDate = cleaning.value;
                // Reset handover if earlier
                if (state.handoverDate && state.handoverDate < state.cleaningDate) {
                    state.handoverDate = '';
                    const h = $('#calc-handover-date');
                    if (h) h.value = '';
                }
                const hEl = $('#calc-handover-date');
                if (hEl) hEl.min = state.cleaningDate;
                const err = $('[data-cleaning-date-error]');
                if (err) err.classList.add('hidden');
            });
        }
        const handover = $('#calc-handover-date');
        if (handover) {
            handover.min = today;
            handover.addEventListener('change', () => {
                state.handoverDate = handover.value;
                const err = $('[data-handover-date-error]');
                if (err) err.classList.add('hidden');
            });
        }
        const handoverTime = $('#calc-handover-time');
        if (handoverTime) {
            handoverTime.addEventListener('change', () => { state.handoverTime = handoverTime.value; });
        }
        const notFixed = $('#calc-handover-not-fixed');
        if (notFixed) {
            notFixed.addEventListener('change', () => {
                state.handoverNotFixed = notFixed.checked;
                if (state.handoverNotFixed) {
                    state.handoverDate = '';
                    state.handoverTime = '';
    ...
    function wireStep4() {
        ...
        if (sameChk) {
            sameChk.addEventListener('change', () => {
                ...
                // Toggle required attribute on object fields
                ['bk-ostreet', 'bk-oplz', 'bk-ocity'].forEach((id) => {
    ...
    function populateHiddenInputs() {
        ...
        setHidden('Basement', state.selectedAddOns.has('basement') ? 'true' : 'false');
        Object.keys(state.extraCounts).forEach((key) => {
            setHidden(COUNTABLE_FIELD[key], String(state.extraCounts[key] || 0));
        });
        setHidden('CleaningDate', state.cleaningDate || '');
        setHidden('HandoverDate', state.handoverNotFixed ? '' : (state.handoverDate || ''));
        setHidden('HandoverTime', state.handoverNotFixed ? '' : (state.handoverTime || ''));
        setHidden('HandoverDateNotFixed', state.handoverNotFixed ? 'true' : 'false');
    ...
    async function handleBookingSubmit(e) {
        ...
        // Browser native validation first (HTML5 required/pattern)
        ...
        try {
            ...
            // Anti-forgery token — Razor inserts it as __RequestVerificationToken
    ...
    // ===== Bootstrap =====
    function bootstrap() {
        // Pre-fill from URL (?zip=X&room=Y) set by the hero form
        ...
        // Auto-advance to step 2 if both zip + room came in valid from URL
